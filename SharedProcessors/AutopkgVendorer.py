...
class AutopkgVendorer(Processor):
    ...
    input_variables = {
        ...
        "destination_path": {"required": False, "description": "Directory to save files"},
    ...
    def modify_identifier(self, plist_data, new_identifier=None):
    ...
    def process_file(self, session, repo, item_path, item_name, commit_sha, dest_path, convert_to_yaml, new_identifier) -> str:
        ...
        if item_name.endswith(('.recipe')):
            ...
            plist_data = self.modify_identifier(plist_data, new_identifier)

            if convert_to_yaml:
                header = self.generate_comment_header(repo, item_path, commit_sha, "yaml")
                body = plist_yaml_from_dict(plist_data)
                full_contents = header + body
    ...
    def vendor_path(self, session, repo, path, commit_sha, dest_base, rel_base="", convert_to_yaml=False, new_identifier=None):
        ...
        for item in items:
            ...
            if item_type == "dir":
                self.vendor_path(session, repo, item_path, commit_sha, dest_base, rel_path, convert_to_yaml, new_identifier)
            elif item_type == "file":
                vendorer_paths.append(
                    self.process_file(session, repo, item_path, item_name, commit_sha, dest_path, convert_to_yaml, new_identifier)
                )
            else:
